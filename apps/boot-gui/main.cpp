// StrayLight Boot Manager — Wayland + EGL + ImGui application
...
int main(int, char*[]) {
    ...
    Log::init("straylight-boot");
    SL_INFO("StrayLight Boot Manager starting");
    ...
    xdg_toplevel_set_title(ws.toplevel, "StrayLight Boot Manager");
    xdg_toplevel_set_app_id(ws.toplevel, "straylight-boot");
    ...
    straylight::boot::BootState app_state;

    SL_INFO("StrayLight Boot Manager initialized");
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        if (ImGui::Begin("##Main", nullptr, kWinFlags)) {
            straylight::boot::render_boot_panel(app_state);
        }
        ImGui::End();
        ImGui::Render();
    ...
    SL_INFO("StrayLight Boot Manager shutting down");
    ...
    SL_INFO("StrayLight Boot Manager exited cleanly");
    return EXIT_SUCCESS;
}
