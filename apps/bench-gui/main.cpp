// StrayLight Bench — Wayland + EGL + ImGui application
...
int main(int, char*[]) {
    ...
    Log::init("straylight-bench");
    SL_INFO("StrayLight Bench starting");
    ...
    xdg_toplevel_set_title(ws.toplevel, "StrayLight Bench");
    xdg_toplevel_set_app_id(ws.toplevel, "straylight-bench");
    ...
    straylight::bench::BenchState bench_state; bench_state.init();

    SL_INFO("StrayLight Bench initialized");
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        if (ImGui::Begin("##Main", nullptr, kWinFlags)) {
            straylight::bench::render_bench_panel(bench_state);
        }
        ImGui::End();
        ImGui::Render();
    ...
    SL_INFO("StrayLight Bench shutting down");
    ...
    SL_INFO("StrayLight Bench exited cleanly");
    return EXIT_SUCCESS;
}
