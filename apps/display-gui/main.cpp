// StrayLight Display Settings — Wayland + EGL + ImGui application
...
int main(int, char*[]) {
    ...
    Log::init("straylight-display");
    SL_INFO("StrayLight Display Settings starting");
    ...
    xdg_toplevel_set_title(ws.toplevel, "StrayLight Display Settings");
    xdg_toplevel_set_app_id(ws.toplevel, "straylight-display");
    ...
    straylight::display::DisplayState app_state;

    SL_INFO("StrayLight Display Settings initialized");
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        if (ImGui::Begin("##Main", nullptr, kWinFlags)) {
            straylight::display::render_display_panel(app_state);
        }
        ImGui::End();
        ImGui::Render();
    ...
    SL_INFO("StrayLight Display Settings shutting down");
    ...
    SL_INFO("StrayLight Display Settings exited cleanly");
    return EXIT_SUCCESS;
}
