// StrayLight Cron — Wayland + EGL + ImGui application
...
int main(int, char*[]) {
    ...
    Log::init("straylight-cron");
    SL_INFO("StrayLight Cron starting");
    ...
    xdg_toplevel_set_title(ws.toplevel, "StrayLight Cron");
    xdg_toplevel_set_app_id(ws.toplevel, "straylight-cron");
    ...
    straylight::cron::CronState cron_state; cron_state.init();

    SL_INFO("StrayLight Cron initialized");
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        if (ImGui::Begin("##Main", nullptr, kWinFlags)) {
            straylight::cron::render_cron_panel(cron_state);
        }
        ImGui::End();
        ImGui::Render();
    ...
    SL_INFO("StrayLight Cron shutting down");
    ...
    SL_INFO("StrayLight Cron exited cleanly");
    return EXIT_SUCCESS;
}
