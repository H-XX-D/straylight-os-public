// StrayLight Notification Settings — Wayland + EGL + ImGui application
#include "notify_panel.h"
...
int main(int, char*[]) {
    ...
    Log::init("straylight-notify");
    SL_INFO("StrayLight Notification Settings starting");
    ...
    xdg_toplevel_set_title(ws.toplevel, "StrayLight Notification Settings");
    xdg_toplevel_set_app_id(ws.toplevel, "straylight-notify");
    ...
    straylight::notify::NotifyState app_state;

    SL_INFO("StrayLight Notification Settings initialized");
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        if (ImGui::Begin("##Main", nullptr, kWinFlags)) {
            straylight::notify::render_notify_panel(app_state);
        }
        ImGui::End();
        ImGui::Render();
    ...
    SL_INFO("StrayLight Notification Settings shutting down");
    ...
    SL_INFO("StrayLight Notification Settings exited cleanly");
    return EXIT_SUCCESS;
}
