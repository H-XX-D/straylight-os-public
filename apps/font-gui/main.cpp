// StrayLight Font Manager — Wayland + EGL + ImGui application
...
int main(int, char*[]) {
    ...
    Log::init("straylight-fonts");
    SL_INFO("StrayLight Font Manager starting");
    ...
    xdg_toplevel_set_title(ws.toplevel, "StrayLight Font Manager");
    xdg_toplevel_set_app_id(ws.toplevel, "straylight-fonts");
    ...
    straylight::fontmgr::FontState app_state;

    SL_INFO("StrayLight Font Manager initialized");
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        if (ImGui::Begin("##Main", nullptr, kWinFlags)) {
            straylight::fontmgr::render_font_panel(app_state);
        }
        ImGui::End();
        ImGui::Render();
    ...
    SL_INFO("StrayLight Font Manager shutting down");
    ...
    SL_INFO("StrayLight Font Manager exited cleanly");
    return EXIT_SUCCESS;
}
