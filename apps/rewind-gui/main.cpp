// StrayLight Process Rewind — Wayland + EGL + ImGui application
#include "rewind_panel.h"
...
int main(int, char*[]) {
    ...
    Log::init("straylight-rewind");
    SL_INFO("StrayLight Process Rewind starting");
    ...
    xdg_toplevel_set_title(ws.toplevel, "StrayLight Process Rewind");
    xdg_toplevel_set_app_id(ws.toplevel, "straylight-rewind");
    ...
    straylight::rewind::RewindState app_state;

    SL_INFO("StrayLight Process Rewind initialized");
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        if (ImGui::Begin("##Main", nullptr, kWinFlags)) {
            straylight::rewind::render_rewind_panel(app_state);
        }
        ImGui::End();
        ImGui::Render();
    ...
    SL_INFO("StrayLight Process Rewind shutting down");
    ...
    SL_INFO("StrayLight Process Rewind exited cleanly");
    return EXIT_SUCCESS;
}
