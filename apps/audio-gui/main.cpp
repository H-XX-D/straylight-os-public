// StrayLight Audio Control — Wayland + EGL + ImGui application
...
int main(int, char*[]) {
    ...
    Log::init("straylight-audio");
    SL_INFO("StrayLight Audio Control starting");
    ...
    xdg_toplevel_set_title(ws.toplevel, "StrayLight Audio Control");
    xdg_toplevel_set_app_id(ws.toplevel, "straylight-audio");
    ...
    straylight::audio::AudioState app_state;

    SL_INFO("StrayLight Audio Control initialized");
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        if (ImGui::Begin("##Main", nullptr, kWinFlags)) {
            straylight::audio::render_audio_panel(app_state);
        }
        ImGui::End();
        ImGui::Render();
    ...
    SL_INFO("StrayLight Audio Control shutting down");
    ...
    SL_INFO("StrayLight Audio Control exited cleanly");
    return EXIT_SUCCESS;
}
