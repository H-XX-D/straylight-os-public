// StrayLight Probe — Wayland + EGL + ImGui application
...
int main(int, char*[]) {
    ...
    Log::init("straylight-probe");
    SL_INFO("StrayLight Probe starting");
    ...
    xdg_toplevel_set_title(ws.toplevel, "StrayLight Probe");
    xdg_toplevel_set_app_id(ws.toplevel, "straylight-sl-probe");
    ...
    straylight::probe::ScanState scan_state; scan_state.init();

    SL_INFO("StrayLight Probe initialized");
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        if (ImGui::Begin("##Main", nullptr, kWinFlags)) {
            straylight::probe::render_scan_panel(scan_state);
        }
        ImGui::End();
        ImGui::Render();
    ...
    SL_INFO("StrayLight Probe shutting down");
    ...
    SL_INFO("StrayLight Probe exited cleanly");
    return EXIT_SUCCESS;
}
