// StrayLight Network Manager — Wayland + EGL + ImGui application
...
int main(int, char*[]) {
    ...
    Log::init("straylight-network");
    SL_INFO("StrayLight Network Manager starting");
    ...
    xdg_toplevel_set_title(ws.toplevel, "StrayLight Network Manager");
    xdg_toplevel_set_app_id(ws.toplevel, "straylight-network");
    ...
    straylight::network::NetworkState app_state;

    SL_INFO("StrayLight Network Manager initialized");
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        if (ImGui::Begin("##Main", nullptr, kWinFlags)) {
            straylight::network::render_network_panel(app_state);
        }
        ImGui::End();
        ImGui::Render();
    ...
    SL_INFO("StrayLight Network Manager shutting down");
    ...
    SL_INFO("StrayLight Network Manager exited cleanly");
    return EXIT_SUCCESS;
}
