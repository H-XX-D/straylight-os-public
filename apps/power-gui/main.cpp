// StrayLight Power Settings — Wayland + EGL + ImGui application
...
int main(int, char*[]) {
    ...
    Log::init("straylight-power");
    SL_INFO("StrayLight Power Settings starting");
    ...
    xdg_toplevel_set_title(ws.toplevel, "StrayLight Power Settings");
    xdg_toplevel_set_app_id(ws.toplevel, "straylight-power");
    ...
    straylight::power::PowerState app_state;

    SL_INFO("StrayLight Power Settings initialized");
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        if (ImGui::Begin("##Main", nullptr, kWinFlags)) {
            straylight::power::render_power_panel(app_state);
        }
        ImGui::End();
        ImGui::Render();
    ...
    SL_INFO("StrayLight Power Settings shutting down");
    ...
    SL_INFO("StrayLight Power Settings exited cleanly");
    return EXIT_SUCCESS;
}
