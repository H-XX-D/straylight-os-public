// StrayLight Vault — Wayland + EGL + ImGui application
...
int main(int, char*[]) {
    ...
    Log::init("straylight-vault");
    SL_INFO("StrayLight Vault starting");
    ...
    xdg_toplevel_set_title(ws.toplevel, "StrayLight Vault");
    xdg_toplevel_set_app_id(ws.toplevel, "straylight-sl-vault");
    ...
    straylight::vault::VaultState vault_state;

    SL_INFO("StrayLight Vault initialized");
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        if (ImGui::Begin("##Main", nullptr, kWinFlags)) {
            straylight::vault::render_vault_panel(vault_state);
        }
        ImGui::End();
        ImGui::Render();
    ...
    SL_INFO("StrayLight Vault shutting down");
    ...
    SL_INFO("StrayLight Vault exited cleanly");
    return EXIT_SUCCESS;
}
