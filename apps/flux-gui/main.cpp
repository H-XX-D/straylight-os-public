// StrayLight Flux — Wayland + EGL + ImGui application
#include "flux_panel.h"
...
int main(int, char*[]) {
    ...
    Log::init("straylight-flux");
    SL_INFO("StrayLight Flux starting");
    ...
    xdg_toplevel_set_title(ws.toplevel, "StrayLight Flux");
    xdg_toplevel_set_app_id(ws.toplevel, "straylight-flux");
    ...
    straylight::flux::FluxState flux_state; flux_state.init();

    SL_INFO("StrayLight Flux initialized");
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        if (ImGui::Begin("##Main", nullptr, kWinFlags)) {
            straylight::flux::render_flux_panel(flux_state);
        }
        ImGui::End();
        ImGui::Render();
    ...
    SL_INFO("StrayLight Flux shutting down");
    ...
    SL_INFO("StrayLight Flux exited cleanly");
    return EXIT_SUCCESS;
}
