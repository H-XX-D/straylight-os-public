// StrayLight User Management — Wayland + EGL + ImGui application
...
int main(int, char*[]) {
    ...
    Log::init("straylight-users");
    SL_INFO("StrayLight User Management starting");
    ...
    xdg_toplevel_set_title(ws.toplevel, "StrayLight User Management");
    xdg_toplevel_set_app_id(ws.toplevel, "straylight-users");
    ...
    straylight::users::UsersState app_state;

    SL_INFO("StrayLight User Management initialized");
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        if (ImGui::Begin("##Main", nullptr, kWinFlags)) {
            straylight::users::render_users_panel(app_state);
        }
        ImGui::End();
        ImGui::Render();
    ...
    SL_INFO("StrayLight User Management shutting down");
    ...
    SL_INFO("StrayLight User Management exited cleanly");
    return EXIT_SUCCESS;
}
