// apps/photonics-gui/main.cpp
// StrayLight Photonics — Wayland + EGL + ImGui application
...
struct WaylandState {
    ...
    double mx = -1.0, my = -1.0;            // cursor position in surface coords
    bool   mb[3] = { false, false, false }; // left, right, middle
    float  wheel = 0.0f;
};
...
void ptr_enter(void* data, wl_pointer* p, uint32_t serial, wl_surface*, wl_fixed_t sx, wl_fixed_t sy) {
    auto* ws = static_cast<WaylandState*>(data);
    ws->mx = wl_fixed_to_double(sx); ws->my = wl_fixed_to_double(sy);
    // ImGui draws its own cursor, so hide the compositor cursor over our surface.
    wl_pointer_set_cursor(p, serial, nullptr, 0, 0);
}
void ptr_leave(void* data, wl_pointer*, uint32_t, wl_surface*) {
    auto* ws = static_cast<WaylandState*>(data); ws->mx = -1.0; ws->my = -1.0;
}
...
void ptr_button(void* data, wl_pointer*, uint32_t, uint32_t, uint32_t button, uint32_t state) {
    ...
    if (button == BTN_LEFT)        ws->mb[0] = down;
    else if (button == BTN_RIGHT)  ws->mb[1] = down;
    else if (button == BTN_MIDDLE) ws->mb[2] = down;
}
void ptr_axis(void* data, wl_pointer*, uint32_t, uint32_t axis, wl_fixed_t value) {
    auto* ws = static_cast<WaylandState*>(data);
    if (axis == WL_POINTER_AXIS_VERTICAL_SCROLL)
        ws->wheel -= static_cast<float>(wl_fixed_to_double(value)) / 15.0f;
}
...
const wl_pointer_listener pointer_listener_impl = {
    ...
    .axis_value120 = nullptr, .axis_relative_direction = nullptr
};
...
void seat_capabilities(void* data, wl_seat* seat, uint32_t caps) {
    ...
    if ((caps & WL_SEAT_CAPABILITY_POINTER) && !ws->pointer) {
        ...
        wl_pointer_add_listener(ws->pointer, &pointer_listener_impl, ws);
    }
}
...
int main() {
    ...
    xdg_toplevel_set_title(ws.toplevel, "StrayLight Photonics");
    xdg_toplevel_set_app_id(ws.toplevel, "straylight.photonics-gui");
    ...
    // Custom EGL/Wayland surface: nothing manages the OS cursor over the window,
    // so draw ImGui's own software cursor or it vanishes over the app.
    ...
    straylight::photonics::PhotonicsPanel panel;
    panel.init();

    while (g_running.load()) {
        const auto frame_start = std::chrono::steady_clock::now();

        // Read + dispatch incoming Wayland events (pointer, resize) non-blocking.
        struct pollfd pfd = { wl_display_get_fd(ws.display), POLLIN, 0 };
        if (poll(&pfd, 1, 0) > 0 && (pfd.revents & POLLIN)) wl_display_dispatch(ws.display);
        else wl_display_dispatch_pending(ws.display);
        ...
        // Feed Wayland pointer state into ImGui before the new frame.
        io.DeltaTime    = 1.0f / 60.0f;
        io.MousePos     = ImVec2(static_cast<float>(ws.mx), static_cast<float>(ws.my));
        io.MouseDown[0] = ws.mb[0]; io.MouseDown[1] = ws.mb[1]; io.MouseDown[2] = ws.mb[2];
        io.MouseWheel  += ws.wheel; ws.wheel = 0.0f;
        ...
        ImGui::Begin("StrayLight Photonics", nullptr,
