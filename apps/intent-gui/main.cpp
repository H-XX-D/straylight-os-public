// StrayLight Intent Commander — Wayland + EGL + ImGui application
#include "intent_panel.h"
...
int main(int, char*[]) {
    ...
    SL_INFO("StrayLight Intent Commander starting");
    ...
    xdg_toplevel_set_title(ws.toplevel, "StrayLight Intent Commander");
    xdg_toplevel_set_app_id(ws.toplevel, "straylight-intent");
    ...
    straylight::intent::IntentState app_state;

    SL_INFO("StrayLight Intent Commander initialized");
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        if (ImGui::Begin("##Main", nullptr, kWinFlags)) {
            straylight::intent::render_intent_panel(app_state);
        }
        ImGui::End();
        ImGui::Render();
    ...
    SL_INFO("StrayLight Intent Commander shutting down");
    ...
    SL_INFO("StrayLight Intent Commander exited cleanly");
    return EXIT_SUCCESS;
}
