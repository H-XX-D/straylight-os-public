// StrayLight Device Topology — Wayland + EGL + ImGui application
...
int main(int, char*[]) {
    ...
    Log::init("straylight-fabric");
    SL_INFO("StrayLight Device Topology starting");
    ...
    xdg_toplevel_set_title(ws.toplevel, "StrayLight Device Topology");
    xdg_toplevel_set_app_id(ws.toplevel, "straylight-fabric");
    ...
    straylight::fabric::FabricState app_state;

    SL_INFO("StrayLight Device Topology initialized");
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        if (ImGui::Begin("##Main", nullptr, kWinFlags)) {
            straylight::fabric::render_fabric_panel(app_state);
        }
        ImGui::End();
        ImGui::Render();
    ...
    SL_INFO("StrayLight Device Topology shutting down");
    ...
    SL_INFO("StrayLight Device Topology exited cleanly");
    return EXIT_SUCCESS;
}
