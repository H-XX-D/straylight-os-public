// StrayLight Mesh — Wayland + EGL + ImGui application
...
int main(int, char*[]) {
    ...
    Log::init("straylight-mesh");
    SL_INFO("StrayLight Mesh starting");
    ...
    xdg_toplevel_set_title(ws.toplevel, "StrayLight Mesh");
    xdg_toplevel_set_app_id(ws.toplevel, "straylight-sl-mesh");
    xdg_toplevel_set_min_size(ws.toplevel, 1000, 680);
    ...
    straylight::mesh::MeshState mesh_state; mesh_state.init();

    SL_INFO("StrayLight Mesh initialized");
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        if (ImGui::Begin("##Main", nullptr, kWinFlags)) {
            straylight::mesh::render_mesh_panel(mesh_state);
        }
        ImGui::End();
        ImGui::Render();
    ...
    SL_INFO("StrayLight Mesh shutting down");
    ...
    SL_INFO("StrayLight Mesh exited cleanly");
    return EXIT_SUCCESS;
}
