// StrayLight Sandbox — Wayland + EGL + ImGui application
#include "sandbox_panel.h"
...
int main(int, char*[]) {
    ...
    Log::init("straylight-sandbox");
    SL_INFO("StrayLight Sandbox starting");
    ...
    xdg_toplevel_set_title(ws.toplevel, "StrayLight Sandbox");
    xdg_toplevel_set_app_id(ws.toplevel, "straylight-sandbox");
    ...
    straylight::sandbox::SandboxState sbox_state; sbox_state.init();

    SL_INFO("StrayLight Sandbox initialized");
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        if (ImGui::Begin("##Main", nullptr, kWinFlags)) {
            straylight::sandbox::render_sandbox_panel(sbox_state);
        }
        ImGui::End();
        ImGui::Render();
    ...
    SL_INFO("StrayLight Sandbox shutting down");
    ...
    SL_INFO("StrayLight Sandbox exited cleanly");
    return EXIT_SUCCESS;
}
