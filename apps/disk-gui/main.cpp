// StrayLight Disk Manager — Wayland + EGL + ImGui application
...
int main(int, char*[]) {
    ...
    Log::init("straylight-disk");
    SL_INFO("StrayLight Disk Manager starting");
    ...
    xdg_toplevel_set_title(ws.toplevel, "StrayLight Disk Manager");
    xdg_toplevel_set_app_id(ws.toplevel, "straylight-disk");
    ...
    straylight::disk::DiskState app_state;

    SL_INFO("StrayLight Disk Manager initialized");
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        if (ImGui::Begin("##Main", nullptr, kWinFlags)) {
            straylight::disk::render_disk_panel(app_state);
        }
        ImGui::End();
        ImGui::Render();
    ...
    SL_INFO("StrayLight Disk Manager shutting down");
    ...
    SL_INFO("StrayLight Disk Manager exited cleanly");
    return EXIT_SUCCESS;
}
