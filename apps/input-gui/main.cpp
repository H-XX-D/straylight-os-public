// StrayLight Input Settings — Wayland + EGL + ImGui application
...
int main(int, char*[]) {
    ...
    Log::init("straylight-input");
    SL_INFO("StrayLight Input Settings starting");
    ...
    xdg_toplevel_set_title(ws.toplevel, "StrayLight Input Settings");
    xdg_toplevel_set_app_id(ws.toplevel, "straylight-input");
    ...
    straylight::input::InputState app_state;

    SL_INFO("StrayLight Input Settings initialized");
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        if (ImGui::Begin("##Main", nullptr, kWinFlags)) {
            straylight::input::render_input_panel(app_state);
        }
        ImGui::End();
        ImGui::Render();
    ...
    SL_INFO("StrayLight Input Settings shutting down");
    ...
    SL_INFO("StrayLight Input Settings exited cleanly");
    return EXIT_SUCCESS;
}
