// StrayLight Log Viewer — Wayland + EGL + ImGui application
#include "log_panel.h"
...
int main(int, char*[]) {
    ...
    Log::init("straylight-logview");
    SL_INFO("StrayLight Log Viewer starting");
    ...
    xdg_toplevel_set_title(ws.toplevel, "StrayLight Log Viewer");
    xdg_toplevel_set_app_id(ws.toplevel, "straylight-logview");
    ...
    straylight::logview::LogState app_state;

    SL_INFO("StrayLight Log Viewer initialized");
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        if (ImGui::Begin("##Main", nullptr, kWinFlags)) {
            straylight::logview::render_log_panel(app_state);
        }
        ImGui::End();
        ImGui::Render();
    ...
    SL_INFO("StrayLight Log Viewer shutting down");
    ...
    SL_INFO("StrayLight Log Viewer exited cleanly");
    return EXIT_SUCCESS;
}
