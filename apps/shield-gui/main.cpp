// StrayLight Shield — Wayland + EGL + ImGui application
...
int main(int, char*[]) {
    ...
    Log::init("straylight-shield");
    SL_INFO("StrayLight Shield starting");
    ...
    xdg_toplevel_set_title(ws.toplevel, "StrayLight Shield");
    xdg_toplevel_set_app_id(ws.toplevel, "straylight-shield");
    ...
    straylight::shield::ShieldState shield_state; shield_state.init();

    SL_INFO("StrayLight Shield initialized");
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        if (ImGui::Begin("##Main", nullptr, kWinFlags)) {
            straylight::shield::render_shield_panel(shield_state);
        }
        ImGui::End();
        ImGui::Render();
    ...
    SL_INFO("StrayLight Shield shutting down");
    ...
    SL_INFO("StrayLight Shield exited cleanly");
    return EXIT_SUCCESS;
}
