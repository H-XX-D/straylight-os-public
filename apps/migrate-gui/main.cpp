// StrayLight Migrate — Wayland + EGL + ImGui application
...
int main(int, char*[]) {
    ...
    Log::init("straylight-migrate");
    SL_INFO("StrayLight Migrate starting");
    ...
    xdg_toplevel_set_title(ws.toplevel, "StrayLight Migrate");
    xdg_toplevel_set_app_id(ws.toplevel, "straylight-migrate");
    ...
    straylight::migrate::MigrateState migrate_state; migrate_state.init();

    SL_INFO("StrayLight Migrate initialized");
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        if (ImGui::Begin("##Main", nullptr, kWinFlags)) {
            straylight::migrate::render_migrate_panel(migrate_state);
        }
        ImGui::End();
        ImGui::Render();
    ...
    SL_INFO("StrayLight Migrate shutting down");
    ...
    SL_INFO("StrayLight Migrate exited cleanly");
    return EXIT_SUCCESS;
}
