// StrayLight Prediction Dashboard — Wayland + EGL + ImGui application
#include "predict_panel.h"
...
int main(int, char*[]) {
    ...
    SL_INFO("StrayLight Prediction Dashboard starting");
    ...
    xdg_toplevel_set_title(ws.toplevel, "StrayLight Prediction Dashboard");
    xdg_toplevel_set_app_id(ws.toplevel, "straylight-predict");
    ...
    straylight::predict::PredictState app_state;

    SL_INFO("StrayLight Prediction Dashboard initialized");
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        if (ImGui::Begin("##Main", nullptr, kWinFlags)) {
            straylight::predict::render_predict_panel(app_state);
        }
        ImGui::End();
        ImGui::Render();
    ...
    SL_INFO("StrayLight Prediction Dashboard shutting down");
    ...
    SL_INFO("StrayLight Prediction Dashboard exited cleanly");
    return EXIT_SUCCESS;
}
