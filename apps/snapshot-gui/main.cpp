// StrayLight Snapshot — Wayland + EGL + ImGui application
...
int main(int, char*[]) {
    ...
    Log::init("straylight-snapshot");
    SL_INFO("StrayLight Snapshot starting");
    ...
    xdg_toplevel_set_title(ws.toplevel, "StrayLight Snapshot");
    xdg_toplevel_set_app_id(ws.toplevel, "straylight-sl-snapshot");
    ...
    straylight::snapshot::SnapshotState snap_state; snap_state.init();

    SL_INFO("StrayLight Snapshot initialized");
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        if (ImGui::Begin("##Main", nullptr, kWinFlags)) {
            straylight::snapshot::render_snapshot_panel(snap_state);
        }
        ImGui::End();
        ImGui::Render();
    ...
    SL_INFO("StrayLight Snapshot shutting down");
    ...
    SL_INFO("StrayLight Snapshot exited cleanly");
    return EXIT_SUCCESS;
}
