// StrayLight Replay — Wayland + EGL + ImGui application
...
int main(int, char*[]) {
    ...
    Log::init("straylight-replay");
    SL_INFO("StrayLight Replay starting");
    ...
    xdg_toplevel_set_title(ws.toplevel, "StrayLight Replay");
    xdg_toplevel_set_app_id(ws.toplevel, "straylight-replay");
    xdg_toplevel_set_min_size(ws.toplevel, 1000, 640);
    ...
    straylight::replay::TimelineState tl_state; tl_state.init();

    SL_INFO("StrayLight Replay initialized");
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        if (ImGui::Begin("##Main", nullptr, kWinFlags)) {
            straylight::replay::render_timeline_panel(tl_state);
        }
        ImGui::End();
        ImGui::Render();
    ...
    SL_INFO("StrayLight Replay shutting down");
    ...
    SL_INFO("StrayLight Replay exited cleanly");
    return EXIT_SUCCESS;
}
