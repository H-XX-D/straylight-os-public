// StrayLight System Policy — Wayland + EGL + ImGui application
...
int main(int, char*[]) {
    ...
    Log::init("straylight-policy");
    SL_INFO("StrayLight System Policy starting");
    ...
    xdg_toplevel_set_title(ws.toplevel, "StrayLight System Policy");
    xdg_toplevel_set_app_id(ws.toplevel, "straylight-policy");
    ...
    straylight::policy::PolicyState app_state;

    SL_INFO("StrayLight System Policy initialized");
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        if (ImGui::Begin("##Main", nullptr, kWinFlags)) {
            straylight::policy::render_policy_panel(app_state);
        }
        ImGui::End();
        ImGui::Render();
    ...
    SL_INFO("StrayLight System Policy shutting down");
    ...
    SL_INFO("StrayLight System Policy exited cleanly");
    return EXIT_SUCCESS;
}
